...
- `TCP` Connect/Full Open Scan (nmap `-sT`) - port closed: `RST` response
- Stealth `TCP` Scanning Methods (nmap `-sS`)
  ...
  - IDLE/IP ID Header Scan: _use of zombie host to check open/closed port using IPID number check_ (nmap `-sI`)
- `UDP` Scanning: _no response if port is open, `ICMP` port unreachable message otherwise_ (nmap `-sU`)
- `SCTP` Scanning:
...
- ip spoofing detection tencniques:
  ...
  - tcp flow
- creating custom packets (namp `--data-string`)
